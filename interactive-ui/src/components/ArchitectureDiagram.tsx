...
const ArchitectureDiagram: React.FC<ArchitectureDiagramProps> = ({ activeFlow }) => {
  ...
  // Tooltip state
  const [tooltip, setTooltip] = useState<{ visible: boolean; x: number; y: number; content: string; title: string; align: 'left' | 'right' } | null>(null);
  ...
  const handleMouseEnter = (e: React.MouseEvent, type: 'user' | 'agent' | 'obo') => {
  ...
  return (
    <div ref={containerRef} className="w-full h-full min-h-[500px] flex items-center justify-center bg-white rounded-xl shadow-sm border border-slate-200 p-4 overflow-hidden relative">
      <style>{`
        @keyframes flow {
      ...
      {tooltip && (
        ...
        >
          <div className="bg-slate-800 text-white text-[10px] p-3 rounded-lg shadow-2xl font-mono border border-slate-600 max-w-xs overflow-hidden backdrop-blur-sm bg-opacity-95">
            <div className="font-bold border-b border-slate-600 pb-1 mb-2 text-amber-400 flex justify-between items-center">
              {tooltip.title}
              <span className="text-[9px] text-slate-400 font-normal">JWT Claims</span>
            </div>
            <pre className="whitespace-pre-wrap break-all leading-tight text-slate-300">
              {tooltip.content}
            </pre>
          </div>
        </div>
      ...
      <svg width="850" height="450" viewBox="0 0 850 450" className="w-full max-w-5xl">
        ...
        <g transform="translate(260, 60)">
          ...
          <rect x="-70" y="-30" width="140" height="70" rx="4" fill={colors.auth} stroke={colors.linkAuth} strokeWidth="2" />
          <text x="0" y="-5" textAnchor="middle" className="text-xs font-bold fill-amber-800">Identity Server</text>
          <text x="0" y="15" textAnchor="middle" className="text-[10px] fill-amber-700">(Asgardeo)</text>
        ...
        {isOBO && (
          <>
            ...
            <path
              ...
              markerEnd="url(#arrowhead-pink)"
            />
            <text x="210" y="240" textAnchor="middle" className="text-[9px] fill-pink-600 font-bold bg-white p-1">Login Request</text>
        ...
        {(isAgent || isOBO) && (
          <>
            <circle r="5" fill={isOBO ? colors.tokenOBO : colors.linkAgent}>
              <animateMotion
                dur="1.5s"
                repeatCount="indefinite"
